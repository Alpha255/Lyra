    ...
    bool bParentEnabled) const
{
    ...
    if (bDrawMarkers)
    {
        const FLinearColor MarkerColor = bSetColorAndOpacity ?
            ColorAndOpacity.Get().GetColor(InWidgetStyle) : (InWidgetStyle.GetColorAndOpacityTint() * MarkerBrush->GetTint(InWidgetStyle));
        ...
        if (MarkerColor.A > KINDA_SMALL_NUMBER)
        {
            ...
            {
                auto MarkerTransform = GetMarkerRenderTransform(Marker, BaseRadius, ApplicationScale);
...
FSlateRenderTransform SCircumferenceMarkerWidget::GetMarkerRenderTransform(const FCircumferenceMarkerEntry& Marker, const float BaseRadius, const float HUDScale) const
{
    ...
    FSlateRenderTransform RotateAboutOrigin(Concatenate(
        FVector2D(-MarkerBrush->ImageSize.X * 0.5f, -MarkerBrush->ImageSize.Y * 0.5f), // Translate to origin
        FQuat2D(LocalRotationRadians),                                                 // Rotate
        FVector2D(MarkerBrush->ImageSize.X * 0.5f, MarkerBrush->ImageSize.Y * 0.5f))); // Translate back
